      ...
      <div className="bg-roof-50 p-2 border-b">
        <Tabs defaultValue={String(activeTab)} onValueChange={(value) => setActiveTab(Number(value))}>
          <ScrollArea className="w-full">
            <TabsList className="w-full justify-start bg-roof-50 p-1 flex space-x-2 overflow-x-auto">
              {days.map((day, idx) => (
                <TabsTrigger 
                  key={day.label} 
                  value={String(idx)}
                  className="px-3 sm:px-4 py-2 data-[state=active]:bg-white data-[state=active]:text-roof-700 data-[state=active]:shadow-sm whitespace-nowrap flex-shrink-0"
                >
